...
// about hissesinin deyisilmesi kodlari
$('.box2').click(function () {
    $('.box2').addClass('animation4');
    $('.box1').removeClass('animation4');
    $('.about-features').show();
    $('.about-product').hide();
});

$('.box1').click(function () {
    $('.box1').addClass('animation4');
    $('.box2').removeClass('animation4');
    $('.about-features').hide();
    $('.about-product').show();
});
...
// şəkillərin ana divə keçməsi kodları
$('.card').click(function() {
    const newSrc = $(this).find('img').attr('src');
    $('.box').find('img').attr('src', newSrc);
});



document.body.onclick = e =>{
    if (e.target !== searchModal && e.target !== searchInput && !searchModal.contains(e.target) && e.target !== select1) {
